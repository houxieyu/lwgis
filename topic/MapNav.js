var featureURL = "http://124.133.27.90:6080/arcgis/rest/services/lwbj/MapServer/";
var basemapURL = "http://124.133.27.90:6080/arcgis/rest/services/lwbj/MapServer";
var map;
var featureLayer;
var datas;
var curdzm = '3712';
...
var curField;
var zbLists = [
                { name: "农民生活水平", data: ["农业人口数", "小汽车", "摩托电瓶车", "沐浴热水器", "空调", "电冰箱", "电视机", "电脑", "手机", "上网手机"] },
                { name: "种植业", data: ["农业人口数", "玉米播种面积", "小麦播种面积", "马铃薯播种面积", "蔬菜播种面积", "生姜播种面积", "大蒜播种面积"] },
                { name: "林业", data: ["农业人口数", "苹果种植面积", "梨种植面积", "红枣种植面积", "桃种植面积", "葡萄种植面积"] },
                { name: "畜牧业", data: ["农业人口数", "肉牛年末存栏", "生猪年末存栏", "绵羊年末存栏", "山羊年末存栏", "肉鸡年末存栏"] },
                { name: "渔业", data: ["农业人口数", "淡水养殖面积", "池塘养殖面积","渔业机动船"] }
];
var zbList;
var title;
var qxdzmlist = [];
...
function InitChart() {
    ...
    option_pm.parallelAxis.push({
        dim: 0, name: '地区', inverse: true, nameLocation: 'start', data: [],
        axisLabel: { margin: -40 }, type: 'category'
    });
    for (var i = 0; i < zbList.length; i++) {
        option_pm.parallelAxis.push({ dim: i + 1, name: zbList[i], inverse: false, nameLocation: 'end' });
    }
}
...
function InitMap(mapid, addstr, bAddLayer) {
    ...
    //显示底图
    require([
      "esri/map", "esri/layers/ArcGISDynamicMapServiceLayer"], function (Map, ArcGISDynamicMapServiceLayer) {
          map = new Map(mapid, mapoption);
          agoLayer = new ArcGISDynamicMapServiceLayer(basemapURL, { "opacity": opacity });
...
function AddFeatureLayer(layerno, filterdzm, bRender) {
  ...
  "esri/symbols/SimpleFillSymbol", "esri/Color", "esri/tasks/query", "esri/symbols/TextSymbol", "esri/layers/LabelClass",
  "dojo/domReady!"
    ], function (
  Map, FeatureLayer, Graphic, SimpleLineSymbol, SimpleFillSymbol, Color, Query, TextSymbol, LabelClass
) {
        if (featureLayer != null)
            map.removeLayer(featureLayer);
        var wheresql = 'AREA_CODE like \'' + filterdzm + '%\'';
        if(filterdzm.substr(0,5)=='37121'){
            wheresql = '';
...
function GetColorByHSL(value, range) {    //[0,0,250] [0,0,50]
    ...
    var rgb = Please.HSV_to_RGB({ h: gval, s: 1, v: 1 })
...
function GetColor(value, range) {
    //[250,250,250] [0,0,250]
    var lowval = 255; var highval = 0;
    var posval = (value - range.min) / (range.max - range.min);
    var gval = Math.round(lowval - (lowval - highval) * posval);
    return [gval, gval, lowval, 0.9];
}
function BuildRender(field, range, renderer, colortype, Color, SimpleFillSymbol) {
    var lowc = 10; var highc = 255;

    $.each(datas, function (n, row) {
        var renderColor;
        if (colortypes == 1) renderColor = new Color(GetColor(row[field], range));
        else if (colortypes == 2) renderColor = new Color(GetColorByHSL(row[field], range));
        renderer.addValue(zfill(row['区划码'], 12), new SimpleFillSymbol().setColor(renderColor));
    });
}

function RenderLayer(dzm, field, colortype) {
    if (dzm == null) dzm = '370811000000';
    require([
